...
import { addDataToMap, setMapInfo, wrapTo, setLocale as setKeplerMapLocale, registerEntry } from '@kepler.gl/actions';
...
export const keplerReducer: Reducer<DatatlasGlInstances> = keplerGlReducer
  ...
  .plugin({
    ...
    ['@@kepler.gl/LAYER_CONFIG_CHANGE']: (state, action: { oldLayer: Layer; newConfig: Partial<LayerBaseConfig> }) => {
...
export const getConversionActions = (keplerId: string, savedMap: SavedMap, locale = getDefaultLocale()) => {
  const loadedMap = keplerMapFactory.load(savedMap);
  const wrapToMap = wrapTo(keplerId);
  return [
    registerMap(keplerId),
    wrapToMap(addDataToMap({ config: loadedMap.config || undefined, datasets: loadedMap.datasets || [] })),
