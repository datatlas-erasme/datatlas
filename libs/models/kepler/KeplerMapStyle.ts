...
export class KeplerMapStyle implements SavedMapStyle {
  ...
  constructor(styleType: BaseMapStyle['id'] = 'dark', mapStyles?: SavedCustomMapStyle) {
    this.styleType = styleType;
    this.mapStyles = mapStyles || KeplerMapStyle.createMapStyles();
  }
  ...
  static createMapStyles() {
    return KeplerMapStyle.DEFAULT_MAP_STYLES.reduce(
      (accu: SavedCustomMapStyle, curr: BaseMapStyle) => ({
        ...accu,
        [curr.id]: { ...curr, accessToken: process.env.REACT_APP_MAPBOX_ACCESS_TOKEN as string, custom: false },
      }),
      {}
    );
  }
